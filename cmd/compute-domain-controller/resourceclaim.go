...
type ResourceClaimManager struct {
	...
	listComputeDomain ListComputeDomainsFunc
}

func NewResourceClaimManager(config *ManagerConfig, getComputeDomain GetComputeDomainFunc, listComputeDomain ListComputeDomainsFunc) *ResourceClaimManager {
	...
	m := &ResourceClaimManager{
		...
		listComputeDomain: listComputeDomain,
	}
...
func (m *ResourceClaimManager) Start(ctx context.Context) error {
	...
	_, err := m.informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		DeleteFunc: func(obj any) {
			m.config.workQueue.EnqueueRawWithKey(obj, "resourceclaim", m.onDelete)
		},
	})
	if err != nil {
...
func (m *ResourceClaimManager) addOrUpdate(ctx context.Context, obj any) error {
	...
	if err != nil {
		klog.Errorf("failed to assert Namespace for computeDomain with domainID %s and ResourceClaim %s/%s: %s", domainID, rc.Namespace, rc.Name, err.Error())
		return nil
	}
...
func (m *ResourceClaimManager) onDelete(ctx context.Context, obj any) error {
	...
	cds, err := m.listComputeDomain()
	if err != nil {
		return fmt.Errorf("error list ComputeDomain: %w", err)
	}

	for _, cd := range cds {
		for _, claim := range cd.Status.ResourceClaims {
			if claim.Name == rc.Name {
				if err = m.RemoveResourceClaimName(ctx, string(cd.UID), rc.Name); err != nil {
					return fmt.Errorf("error removing ResourceClaim in ComputeDomain: %w", err)
				}
				klog.Infof("Successfully removed ResourceClaim (%s/%s) from ComputeDomain %s", rc.Namespace, rc.Name, string(cd.UID))
				return nil
			}
		}
	}
...
func (m *ResourceClaimManager) RemoveResourceClaimName(ctx context.Context, cdUID, rcName string) error {
	cd, err := m.getComputeDomain(cdUID)
	if err != nil {
		return fmt.Errorf("error getting ComputeDomain: %w", err)
	}
	...
	var filteredResourceClaims []*nvapi.ComputeDomainResourceClaim
	for _, rc := range cd.Status.ResourceClaims {
		if rc.Name == rcName {
			continue
		}
		filteredResourceClaims = append(filteredResourceClaims, rc)
	}

	// Check if update is needed
	if m.rcEqual(cd.Status.ResourceClaims, filteredResourceClaims) {
		return nil
	}
...
func (m *ResourceClaimManager) rcEqual(a, b []*nvapi.ComputeDomainResourceClaim) bool {
	aMap := make(map[string]nvapi.ComputeDomainResourceClaim)
	for _, rc := range a {
		aMap[rc.Name] = *rc
	}
	bMap := make(map[string]nvapi.ComputeDomainResourceClaim)
	for _, rc := range b {
		bMap[rc.Name] = *rc
	}
