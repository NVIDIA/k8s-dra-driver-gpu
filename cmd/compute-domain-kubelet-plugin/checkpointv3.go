...
type PreparedDeviceV3 struct {
	Channel *PreparedComputeDomainChannelV3 `json:"channel,omitempty"`
	Daemon  *PreparedComputeDomainDaemonV3  `json:"daemon,omitempty"`
}

type PreparedComputeDomainChannelV3 struct {
	Info   *ComputeDomainChannelInfo `json:"info,omitempty"`
	Device *CheckpointDevice         `json:"device,omitempty"`
}

type PreparedComputeDomainDaemonV3 struct {
	Info   *ComputeDomainDaemonInfo `json:"info,omitempty"`
	Device *CheckpointDevice        `json:"device,omitempty"`
}
...
func preparedDeviceToV3(d PreparedDevice) PreparedDeviceV3 {
	...
	if d.Channel != nil {
		out.Channel = &PreparedComputeDomainChannelV3{
			Info:   d.Channel.Info,
			Device: NewCheckpointDevice(d.Channel.Device),
		}
	}
	if d.Daemon != nil {
		out.Daemon = &PreparedComputeDomainDaemonV3{
			Info:   d.Daemon.Info,
			Device: NewCheckpointDevice(d.Daemon.Device),
		}
	}
	return out
}
...
func preparedDeviceFromV3(d PreparedDeviceV3) PreparedDevice {
	out := PreparedDevice{}
	if d.Channel != nil {
		out.Channel = &PreparedComputeDomainChannel{
			Info:   d.Channel.Info,
			Device: d.Channel.Device.ToKubeletPluginDevice(),
		}
	}
	if d.Daemon != nil {
		out.Daemon = &PreparedComputeDomainDaemon{
			Info:   d.Daemon.Info,
			Device: d.Daemon.Device.ToKubeletPluginDevice(),
		}
	}
	return out
}
