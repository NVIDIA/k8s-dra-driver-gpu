...
func GetOpaqueDeviceConfigs(
	...
	for _, config := range candidateConfigs {
		...
		if err != nil {
			// Permanently bad opaque config: i) do not retry preparing this
			// resource internally and ii) return notion of permanent error to
			// kubelet, to give it an opportunity to play this error back to the
			// user so that it becomes actionable.
