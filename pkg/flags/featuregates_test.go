...
var (
	...
	testValidDisabledFeature = featuregates.ExampleFeature
	...
	testValidEnabledFeature = logsapi.ContextualLogging
)
...
func TestFeatureGateConfigSetFromMap(t *testing.T) {
	t.Run("EnableDisabledFeature", func(t *testing.T) {
		...
		// Test enabling a disabled feature - we know it starts as false
		require.False(t, featuregates.Enabled(testValidDisabledFeature), "Disabled feature should start as false")
	...
	t.Run("DisableEnabledFeature", func(t *testing.T) {
		...
		// Test with an enabled feature - we know it starts as true
		require.True(t, featuregates.Enabled(testValidEnabledFeature), "Enabled feature should start as true")

		// Disable it through SetFromMap (simulating CLI input)
		err := fg.SetFromMap(map[string]bool{
		...
		// Reset to original enabled state
		err = fg.SetFromMap(map[string]bool{
			string(testValidEnabledFeature): true,
		})
		require.NoError(t, err, "Should be able to reset feature to enabled state")
	})
}
...
func TestFeatureGateConfigToMap(t *testing.T) {
	...
	require.Contains(t, result, "ExampleFeature", "Should contain ExampleFeature")
	require.Equal(t, false, result["ExampleFeature"], "ExampleFeature should have default value false")
	require.Contains(t, result, "ContextualLogging", "Should contain ContextualLogging")
	require.Equal(t, true, result["ContextualLogging"], "ContextualLogging should have overridden value true")
}
